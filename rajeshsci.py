...
raj = StringVar()
raj= Entry(window, width=31, textvariable=raj, relief=SUNKEN, font="Roboto 20")
raj.grid(row=0, column=0, columnspan=10, padx=11, pady=12)
...
def sciCal(event):
    ...
    raj.delete(0, END)
    if text == "sin":
        raj.insert(0, m.sin(float(val)))
    elif text == "cos":
        raj.insert(0, m.cos(float(val)))
    elif text == "tan":
        raj.insert(0, m.tan(float(val)))
    elif text == "log":
        if (float(val) <= 0.00):
            raj.insert(0, "Not Possible")
        else:
            raj.insert(0, m.log10(float(val)))
    elif text == "ln":
        if (float(val) <= 0.00):
            raj.insert(0, "Not Possible")
        else:
            raj.insert(0, m.log(float(val)))
    elif text == "√":
        raj.insert(0, m.sqrt(float(val)))
    elif text == "!":
        raj.insert(0, m.factorial(int(val)))
    elif text == "rad":
        raj.insert(0, m.radians(float(val)))
    elif text == "deg":
        raj.insert(0, m.degrees(float(val)))
    elif text == "1/x":
        if (val == "0"):
            raj.insert(0, "ꝏ")
        else:
            raj.insert(0, 1 / float(val))
    elif text == "π":
        if val == "":
            ans = str(m.pi)
            raj.insert(0, ans)
        ...
        if val == "":
            raj.insert(0, str(m.e))
        else:
            raj.insert(0, str(float(val) * (m.e)))
